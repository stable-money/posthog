...
export const wizardLibraryLogic = kea<wizardLibraryLogicType>([
    ...
    selectors({
        ...
        connectGitHubUrl: [
            () => [],
            (): string => api.integrations.authorizeUrl({ kind: 'github', next: urls.wizard() }),
        ],
    ...
    listeners(({ actions, values }) => ({
        ...
        createRun: () => {
            ...
            if (!projectId || !program) {
                return
            }

            const workspace: WizardRunCreateRequestApi['workspace'] =
                values.libraryEnvironment === 'cloud'
                    ? { type: 'git_repository', repository: values.repository }
                    : { type: 'local_folder', project_name: 'Local project' }
            ...
            actions.createRunRequest({
                ...
                body: {
                    ...
                    workspace,
                    ...(values.libraryEnvironment === 'cloud' ? { idempotency_key: crypto.randomUUID() } : {}),
                },
            })
        },
        createRunRequestSuccess: () => {
