...
def ast_select_tests(changed_files: list[str], features_by_path: dict[str, TestFeatures]) -> AstSelection:
    ...
    # A deleted test file must not reach pytest, which hard-errors on a missing
    # positional arg — git diff lists it as changed even though it no longer exists.
    changed_tests = [path for path in changed_files if _is_test_file(path) and (REPO_ROOT / path).is_file()]
