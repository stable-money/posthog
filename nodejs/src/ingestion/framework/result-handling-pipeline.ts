 ...
 *
 * The unacked results (TIMEOUT, REJECTED) are counted and nothing else: the
 * message is not acked, so its source redelivers it and producing anything
 * here would duplicate what the redelivery produces.
 */
...
function resultDetails<T, R extends string>(result: PipelineResult<T, R>): { result: string; details: string } {
    switch (result.type) {
        ...
        case PipelineResultType.REJECTED:
            return { result: 'rejected', details: result.reason }
    }
}
