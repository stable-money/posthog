...
export const batchBudgetCheckpointCounter = new Counter({
    ...
    help: 'Elements a budget checkpoint cut off, by checkpoint and step. In shadow mode nothing is cut off and the count says what enforcement would have done',
    labelNames: ['checkpoint', 'step_name', 'mode'],
})

/** Where a budget checkpoint sits: before an element step or before a chunk step. */
export type BudgetCheckpoint = 'step' | 'chunk'

export function recordBudgetCheckpoint(checkpoint: BudgetCheckpoint, stepName: string, enforce: boolean): void {
    batchBudgetCheckpointCounter.labels({ checkpoint, step_name: stepName, mode: budgetMode(enforce) }).inc()
}

export function budgetMode(enforce: boolean): string {
    return enforce ? 'enforced' : 'shadow'
}
